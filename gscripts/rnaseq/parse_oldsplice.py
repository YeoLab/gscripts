        ...
        for i, geneItem in enumerate(sampleData):
            ...
            for splicetype in splicetypes:
                ...
                for loc in geneItem[splicetype]:
                    ...
                    data[splicetype][gene][loc][sampleLabel] =
                    geneItem[splicetype][loc]
...
def main(options):
    ...
    if "SE" in spliceData:
        with open("%s.SE.table.txt" % options.name, 'w') as out:
            ...
            for sample in samples:
                ...
                header.extend([sample_label + "_IN", sample_label + "_EX",
                               sample_label + "_psi",
                               sample_label + "_BODY_RPK",
                               sample_label + "_UP_RPK",
                               sample_label + "_DOWN_RPK",
                               sample_label + "_UPINTRON_RPK",
                               sample_label + "_DOWNINTRON_RPK"])
            out.write("\t".join(header) + "\n")
            for gene in spliceData["SE"]:
                for loc in spliceData["SE"][gene]:
                    chr, start, stop, name, score, strand =
                    annotation[gene]["SE"][loc]["bedTrack"].split("\t")
                    wholeLoc = start + "-" + stop
                    ...
                    for sample in samples:
                        sample_label = sample[1]
                        sample_IN = spliceData["SE"][gene][loc][sample_label][
                            "IN"]
                        sample_EX = spliceData["SE"][gene][loc][sample_label][
                            "EX"]
                        sample_BODY = spliceData["SE"][gene][loc][sample_label][
                            "BODY_RPK"]
                        sample_UP = spliceData["SE"][gene][loc][sample_label][
                            "UP_RPK"]
                        sample_DOWN = spliceData["SE"][gene][loc][sample_label][
                            "DOWN_RPK"]
                        try:
                            sample_UPI =
                            spliceData["SE"][gene][loc][sample_label]["UPI_RPK"]
                            sample_DOWNI =
                            spliceData["SE"][gene][loc][sample_label][
                                "DOWNI_RPK"]
                        except:
                            import pdb;
                        ...
                        writeMe.extend(
                            [sample_IN, sample_EX, psi, sample_BODY, sample_UP,
                             sample_DOWN, sample_UPI, sample_DOWNI])
            ...
            with open(sample_label + ".oldsplice.SE", 'w') as SEout:
                for gene in spliceData["SE"]:
                    for loc in spliceData["SE"][gene]:
                        ...
                        sample_UPI = spliceData["SE"][gene][loc][sample_label][
                            "UPI_RPK"]
                        sample_DOWNI =
                        spliceData["SE"][gene][loc][sample_label]["DOWNI_RPK"]
                        psi = calculate_psi_SE(sample_IN, sample_EX)

                        wholeLoc = start + "-" + stop
                        line = "\t".join(map(str, [gene,
                                                   annotation[gene]["SE"][loc][
                                                       'prettyName'],
                                                   (
                                                       chr + ":" + wholeLoc + "|" + strand),
                                                   loc,
                                                   sample_IN, sample_EX,
                                                   "%1.2f" % (psi), sample_BODY,
                                                   sample_UP, sample_DOWN,
                                                   sample_UPI, sample_DOWNI]))
                        SEout.write(line + "\n")
    ...
    if "MXE" in spliceData:
        with open("%s.MXE.table.txt" % options.name, 'w') as out:
            ...
            for sample in samples:
                sample_label = sample[1]
                header.extend([sample_label + "_A", sample_label + "_B",
                               sample_label + "_psi"])
            out.write("\t".join(header) + "\n")
            ...
            for gene in spliceData["MXE"]:
                for loc in spliceData["MXE"][gene]:
                    chr, start, stop, name, score, strand =
                    annotation[gene]["MXE"][loc]["bedTrack"].split("\t")
                    wholeLoc = start + "-" + stop
                    ...
                    for sample in samples:
                        sample_label = sample[1]
                        sample_A = spliceData["MXE"][gene][loc][sample_label][
                            "A"]
                        sample_B = spliceData["MXE"][gene][loc][sample_label][
                            "B"]
    ...
    if "SE" in spliceData:
        ...
        if len(samples) == 2 and options.compare == True:
            ...
            header = "\t".join(
                ["Gene", "ExonName", "Eventloc", "Exonloc", "p-value", "Test",
                 "Testdetails", "significant?", "direction", s1_inlabel,
                 s1_exlabel, s2_inlabel, s2_exlabel, s1_psilabel,
                 s2_psilabel]) + "\n"

            SEout.write(header)
            for gene in spliceData["SE"]:
                for loc in spliceData["SE"][gene]:
                    sample1_IN = spliceData["SE"][gene][loc][samples[0][1]][
                        "IN"]
                    sample1_EX = spliceData["SE"][gene][loc][samples[0][1]][
                        "EX"]
                    sample2_IN = spliceData["SE"][gene][loc][samples[1][1]][
                        "IN"]
                    sample2_EX = spliceData["SE"][gene][loc][samples[1][1]][
                        "EX"]
                    sampledata = np.array(
                        [[sample1_IN, sample1_EX], [sample2_IN, sample2_EX]])
                    ...
                    if np.any(sampledata < 5):
                        ...
                        test = "chi"
                        chi2, p, dof, exp = scipy.stats.chi2_contingency(
                            sampledata, correction=True)
                    ...
                    if p < pval_cutoff:
                        ...
                        bedline = "\t".join(
                            [chr, start, stop, gene, sci_pval, strand, start,
                             stop, color]) + "\n"
                        SEbed.write(bedline)
                    wholeLoc = start + "-" + stop
                    ...
                    line = "\t".join(map(str, [gene,
                                               annotation[gene]["SE"][loc][
                                                   'prettyName'], (
                                                   chr + ":" + wholeLoc + "|" + strand),
                                               loc, p, test, testdetails, issig,
                                               direction, sample1_IN,
                                               sample1_EX, sample2_IN,
                                               sample2_EX, "%1.2f" % (psi1),
                                               "%1.2f" % (psi2)]))
                    SEout.write(line + "\n")
            SEout.close()
    ...
    if "MXE" in spliceData:
        ...
        if len(options.samples) == 2:
            ...
            header = "\t".join(
                ["Gene", "ExonName", "Eventloc", "Exonloc", "p-value", "Test",
                 "Testdetails",
                 "significant?", "direction", s1_inlabel, s1_exlabel,
                 s2_inlabel,
            ...
            for gene in spliceData["MXE"]:
                for loc in spliceData["MXE"][gene]:
                    chr, start, stop, name, score, strand =
                    annotation[gene]["MXE"][loc]["bedTrack"].split("\t")
                    sample1_IN = spliceData["MXE"][gene][loc][samples[0][1]][
                        "A"]
                    sample1_EX = spliceData["MXE"][gene][loc][samples[0][1]][
                        "B"]
                    sample2_IN = spliceData["MXE"][gene][loc][samples[1][1]][
                        "A"]
                    sample2_EX = spliceData["MXE"][gene][loc][samples[1][1]][
                        "B"]
                    sampledata = np.array(
                        [[sample1_IN, sample1_EX], [sample2_IN, sample2_EX]])
                    ...
                    line = "\t".join(map(str, [gene, (
                        chr + ":" + wholeLoc + "|" + strand), loc, p, test,
                                               testdetails, issig, direction,
                                               sample1_IN, sample1_EX,
                                               sample2_IN, sample2_EX,
                                               "%1.2f" % (psi1),
                                               "%1.2f" % (psi2)]))
...
if __name__ == "__main__":
    ...
    parser.add_option("--sample", nargs=2, action="append", dest="samples",
                      help="Two values: --sample filename label")
    parser.add_option("--compare", dest="compare", default=False,
                      action="store_true",
                      help="run 2-way comparison (only works when 2 samples are provided)")
    parser.add_option("--pvalue", dest="pval", default=0.05,
                      help="p-value cutoff for chi2 or fisher exact")
    parser.add_option("--splice_type", dest="splicetype", default=["SE", "MXE"],
                      action="append")
