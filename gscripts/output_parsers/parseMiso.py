...
def uncertainty(msmts):
    ...
    low = msmts[:,1]
    high = msmts[:,2]
    stdevs = np.max(np.c_[np.abs(means - low), np.abs(means-high)], axis=1)# std of msmts
    term1 = np.sum(stdevs**2)
    return np.array([sumMean, max(0, sumMean - np.sqrt(term1)),  min(1, sumMean + np.sqrt(term1))])
...
def parseMisoComparison(line):
    ...
    counts1 = tuple([(tuple(map(int, i.split(":")[0].split(","))), int(i.split(":")[1])) \
                    for i in [i.replace("(", "").replace(")", "") for i in counts1.split(",(")]])

    counts2 = tuple([(tuple(map(int, i.split(":")[0].split(","))), int(i.split(":")[1])) \
                    for i in [i.replace("(", "").replace(")", "") for i in counts2.split(",(")]])    
    ...
    type1_1 = uncertainty(np.c_[np.array(means1)[isoformTypes == 0], np.array(low1)[isoformTypes == 0],\
                        np.array(high1)[isoformTypes == 0]])
    type2_1 = uncertainty(np.c_[np.array(means1)[isoformTypes == 1], np.array(low1)[isoformTypes == 1],\
                        np.array(high1)[isoformTypes == 1]])

    type1_2 = uncertainty(np.c_[np.array(means2)[isoformTypes == 0], np.array(low2)[isoformTypes == 0],\
                        np.array(high2)[isoformTypes == 0]])
    type2_2 = uncertainty(np.c_[np.array(means2)[isoformTypes == 1], np.array(low2)[isoformTypes == 1],\
                        np.array(high2)[isoformTypes == 1]])    


    assigned_counts1 = map(int, [l for sublist in assigned_counts1.split(",") for l in sublist.split(":")])[1::2]
    assigned_counts2 = map(int, [l for sublist in assigned_counts2.split(",") for l in sublist.split(":")])[1::2]    
    ...
    for cat, ct in counts1:
        ...
        countType = str((int(sum(cat * iso0Type) > 0), int(sum(cat * iso1Type) > 0))).replace(" ", "")
        NCts1[countType] += ct
    ...
    return "\t".join(map(str, [event_name, "\t".join(["%.2f" %i for i in type2_1.ravel()]),
...
def parseMisoSummary(line):
    ...
    counts = tuple([(tuple(map(int, i.split(":")[0].split(","))), int(i.split(":")[1])) \
                    for i in [i.replace("(", "").replace(")", "") for i in counts.split(",(")]])
    ...
    type1 = uncertainty(np.c_[np.array(means)[isoformTypes == 0], np.array(low)[isoformTypes == 0],\
                        np.array(high)[isoformTypes == 0]])
    type2 = uncertainty(np.c_[np.array(means)[isoformTypes == 1], np.array(low)[isoformTypes == 1],\
                        np.array(high)[isoformTypes == 1]])
    assigned_counts = map(int, [l for sublist in assigned_counts.split(",") for l in sublist.split(":")])[1::2]
    ...
    return "\t".join(map(str, [event_name, "\t".join(["%.2f" %i for i in type2.ravel()]), ",".join([repExIsoform, repInIsoform]),\
                              NCounts, Nassigned_counts, chrom, strand, repStarts, repEnds]))
