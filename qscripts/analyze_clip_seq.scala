...
class AnalizeCLIPSeq extends QScript {
  ...
  def script() {
    ...
    for (fastq_file: File <- input) {
      ...
      add(new GenomeCoverageBed(inBam = rmDupedBamFile, genomeSize = chr_sizes, bedGraph = bedGraphFilePos, strand = "+", split = true))
      add(new BedGraphToBigWig(bedGraphFilePos, chr_sizes, bigWigFilePos))

      add(new GenomeCoverageBed(inBam = rmDupedBamFile, genomeSize = chr_sizes, bedGraph = bedGraphFileNeg, strand = "-", split = true))
