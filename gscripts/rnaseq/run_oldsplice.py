...
for row, dat in sInfo.iterrows():
    ...
    bam = dat['Bam File']
    out = id + ".splices"
    oldsplice_command = "oldsplice.py -b %s -s hg19 -o %s --splicetypes SE --splicetypes MXE --processors 16" %(bam, out)
    cmds.append(oldsplice_command)
    oldsplice_command = "oldsplice.py -f -b %s -s hg19 -o %s --splicetypes SE --splicetypes MXE --processors 16" %(bam, out.replace(".splices", ".flip.splices")  )
    cmds.append(oldsplice_command)
