...
def correlation(bam_1, bam_2, outbam_name):
    ...
    with pysam.Samfile(bam_1) as bam_1, pysam.Samfile(bam_2) as bam_2:
        ...
        for read in bam_1:
            total_count += 1
            read_start = read.positions[-1] if read.is_reverse else
            read.positions[0]
            ...
            for fetched_read in fetched_reads:
                ...
                fetched_start = fetched_read.positions[
                    -1] if fetched_read.is_reverse else fetched_read.positions[
                    0]

                if read.qname.split(":")[0] == fetched_read.qname.split(":")[
                    0] and read_start == fetched_start:
                    matched_count += 1
                    outbam.write(read)
                    break
    outbam.close()
...
if __name__ == '__main__':
    ...
    outbam = pysam.Samfile(os.path.splitext(options.out_file)[0] + ".sam", "wh",
                           pysam.Samfile(options.bam_1))
    ...
    with open(os.path.join(options.out_file), 'w') as outfile:
        outfile.write("\t".join(
            map(str, [name1, name2, matched_count, total_count])) + "\n")
