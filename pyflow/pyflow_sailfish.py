...
class PyflowSailfish(WorkflowRunner):
    ...
    def workflow(self):
        ...
        for plate in plates:
            for slid in self.slid_mapper[plate]:
                ...
                taskname = "sailfish_%s" % (slid)
                dependencies=()
                sailfish_script=os.path.join(config.bash_src_dir,
                                             "sailfish.bash")
                bash = config.bash
                command = "%s %s %s %s %s %s" % (bash,
                                                 sailfish_script,
                                                 fq1,
                                                 fq2,
                                                 output_dir,
...
if __name__=='__main__':
    ...
    sf = PyflowSailfish(options.source,
                        options.dest,
                        options.plate,
                        options.slids)
