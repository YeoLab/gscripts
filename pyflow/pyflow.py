...
class RunMode(object):
    ...
    data = { "local" : ModeInfo(defaultCores=1,
             ...
             "sge"   : ModeInfo(defaultCores=getSGEJobsDefault(),
                                ...
                                defaultIsRetry=True),
             "torque"   : ModeInfo(defaultCores=getSGEJobsDefault(),
                                defaultMemMbPerCore="unlimited",
                                defaultIsRetry=True)
             }
...
class SGETaskRunner(CommandTaskRunner) :
    ...
    def runOnce(self, retInfo) :
        ...
        self.infoLog("Task submitted to torque queue with job_number: %i" % (self.jobId))
...
class TorqueTaskRunner(CommandTaskRunner) :
    def jobIdParser(self,jobidstr):
        # tscc has num, then queue, then other stuff. strip
        # down to number
        return jobidstr.split(".")[0]
    ...
    def getFullCmd(self):
        ...
        qsubCmd = ["qsub",
                  ...
                  "walltime=72:00:00",
                  "-W",
                  "group_list=autism-group",
                  "-W",
                  "umask=007",
                   "-o", self.wrapFile,
                   "-e", self.wrapFile,
                   "-v", "ID=%s,NAME=%s,PICKLE=%s,RUNDIR=%s" % (self.runid, self.taskStr,self.argFile,os.path.dirname(self.wrapFile)),
                   ]

        
        qsubCmd.extend(self.schedulerArgList)
        qsubCmd.extend(siteConfig.qsubResourceArg(self.nCores, self.memMb))
        bashwrapper="/projects/ps-yeolab/ace_progenity/asd_processing/code_repository/ASDAnalysis/bash_scripts/taskwrapper.bash"
        qsubCmd.append(bashwrapper)
    ...
    def setNewJobId(self, jobId) :
        ...
        command was being submitted, so we must kill the job here:
        """
        self.jobId = self.jobIdParser(jobId)
        if self.stopped(): self._killJob()
    ...
    def runOnce(self, retInfo) :
        ...
        def qcallWithTimeouts(cmd, maxQcallAttempt=1) :
            ...
            for i in range(maxQcallAttempt) :
                ...
                qcall.join(maxQcallWait)
                if not qcall.isAlive() : break
                self.infoLog("Trial %i of torque command has timed out. Killing process for cmd '%s'" % ((i + 1), cmd))
                qcall.killProc()
                self.infoLog("Finished attempting to kill torque command")
        ...
        # No qsub errors detected and an torque job_number is acquired -- success!
        self.infoLog("Task submitted to torque queue with job_number: %s" % (self.jobId))


        # 2) poll jobId until torque indicates it's not running or queued:
        #
        queueStatus = Bunch(isQueued=True, runStartTimeStamp=None)
        ...
        def checkWrapFileRunStart(result) :
            ...
            """
            if not os.path.isfile(self.wrapFile) : return
            if not os.access(self.wrapFile,os.R_OK): return
        ...
        pollCmd = ("/bin/bash", "--noprofile", "-o", "pipefail", "-c", "qstat")
        max_qstat_err = 20
        cur_qstat_err = 0 # sdsc highly unreliable
        ...
        while not self.stopped():
            ...
            job_done = False
            found_tokens = None
            for line in results.outList:
                tokens = line.rstrip().split()
                if len(tokens) != 6:
                    continue
                jobid = self.jobIdParser(tokens[0])
                state = tokens[4]
                if jobid == self.jobId:
                    found_job = True
                    found_tokens = tokens
            ...
            if results.retval != 0:
                ...
                cur_qstat_err = 0
            if not found_job or job_done:
                # all done
                break 
            if cur_qstat_err > max_qstat_err:
                isQstatError = True

            if isQstatError :
                if not results.isComplete :
                    retInfo.taskExitMsg = ["The qstat command for torque job_id %s has timed out for all attempted retries" % (self.jobId)]
                    self._killJob()
                else :
                    retInfo.taskExitMsg = ["Unexpected qstat output or task has entered sge error state. job_id: %s" % (self.jobId)]
                    retInfo.taskExitMsg.extend(["Logging %i line(s) of qstat output below:" % (len(results.outList)) ])
                    retInfo.taskExitMsg.extend([ "[qstat-out] " + line for line in results.outList ])
                    # self._killJob() # leave the job there so the user can better diagnose whetever unexpected pattern has occured
                return retInfo
...
class TaskManager(StoppableThread) :
    ...
    def _getCommandTaskRunner(self, task) :
        ...
        elif param.mode == "torque" :
            TaskRunner = TorqueTaskRunner
        else :
