...
def region_rpk(interval, bam):
    ...
    readCount = bam.count(reference=chrom, start=start,
                          end=stop) #this is not strand-ed
...
def assign_reads(gene, splicedict=None, bam_file=None,
    ...
    subset_reads = bam_fileobj.fetch(reference=chrom, start=tx_start,
                                     end=tx_end)
    ...
    if "SE" in splicedict and "SE" in splicetypes:
        ...
        for loc in splicedict["SE"]:
            ...
            if strand == 1:
                ...
                downIntronLoc = bodyLoc.split("-")[1] + "-" +
                downLoc.split("-")[0]
        else:
            upIntronLoc = bodyLoc.split("-")[1] + "-" + upLoc.split("-")[0]
            downIntronLoc = downLoc.split("-")[1] + "-" +
            bodyLoc.split("-")[0]

    try:
        data["SE"][loc]["BODY_RPK"] = region_rpk(
            pybedtools.Interval(chrom, *map(int, bodyLoc.split("-")),
                                strand=signstrand), bam_fileobj)

        data["SE"][loc]["UP_RPK"] = region_rpk(
            pybedtools.Interval(chrom, *map(int, upLoc.split("-")),
                                strand=signstrand), bam_fileobj)

        data["SE"][loc]["DOWN_RPK"] = region_rpk(
            pybedtools.Interval(chrom, *map(int, downLoc.split("-")),
                                strand=signstrand), bam_fileobj)
...
def retrieve_splicing(species):
    ...
    try:
        info = pickle.load(
            open((basedir + species + ".spliceDict_simple.pickle")))
    except:
        if species == "hg19":
        ...
        for chr in chrs:
            ...
            while not eof:
                ...
                info[gene]["tx_start"] = min(
                    map(int, exonloc.rstrip("|").replace("|", "-").split("-")))
                info[gene]["tx_end"] = max(
                    map(int, exonloc.rstrip("|").replace("|", "-").split("-")))
                line = f.next().strip()
                if line.startswith("<"):
                    while True:
                        ...
                        loc, splicingType, inorout, labels = splicing_labels.split(
                            "\t")
                        exonIndexes = overlap(chromosome + ":" + loc, map(
                            lambda x: chromosome + ":" + x, exons))
                        if len(exonIndexes) > 1:
                            raise Exception
                        try:

                            exN = numbered[exons[exonIndexes[0]]]
                            thisExonNumber = gene + "|" + str(
                                exN - 1) #convert to 0-based exon numbers
                        ...
                        if "SE" in splicingType:
                            ...
                            if not loc in info[gene][splicingType]:
                                ...
                                info[gene][splicingType][loc][
                                    'prettyName'] = thisExonNumber
                                info[gene][splicingType][loc][
                                    "rangestart"] = 10000000000000
                                info[gene][splicingType][loc][
                                    "rangeend"] = -10000000000000
                                info[gene][splicingType][loc][
                                    "bedTrack"] = str()
                            ...
                            for v in versions:
                                ...
                                info[gene][splicingType][loc][
                                    "rangestart"] = min(
                                    info[gene][splicingType][loc]["rangestart"],
                                    (int(locUp) + 1))
                                info[gene][splicingType][loc]["rangeend"] = max(
                                    info[gene][splicingType][loc]["rangeend"],
                                    (int(locDown) + 1))
                                ...
                                if "IN" in inorout:
                                    if info[gene][splicingType][loc]["BODY"]:
                                        ...
                                        (x1, x2, y1, y2) = map(int,
                                                               [x1, x2, y1, y2])
                                        if (y2 - y1) > (
                                                x2 - x1): #if this exon body is bigger than previous versions of this exon, update.
                                            info[gene][splicingType][loc][
                                                "BODY"] = locIn
                                    else:
                                        info[gene][splicingType][loc][
                                    ...
                                    try:
                                        info[gene][splicingType][loc][inorout][
                                            "j" + locUp + ":" + str(int(
                                                exstart) + 1)] += 1#upstream jxn
                                    except:
                                        info[gene][splicingType][loc][inorout][
                                            "j" + locUp + ":" + str(int(
                                                exstart) + 1)] = 1#upstream jxn
                            ...
                            for v in versions:
                                ...
                                exstart, exstop = locIn.split("-")
                                jxns = [
                                    ("j" + locUp + ":" + str(int(exstart) + 1)),
                                    ("j" + exstop + ":" + str(int(exstop) + 1))]
                                for jxn in jxns:
                                    try:
                                        info[gene][splicingType][loc]['jxns'][
                                            jxn] += 1
        ...
        pickle.dump(info,
                    file=open((basedir + species + ".spliceDict_simple.pickle"),
                              "w"))
    return info
...
def main(options):
    ...
    for g in genes:
        args.append(
            [g, splicing[g], bamfile, options.slop, options.flip, splicetypes])
    debug = options.debug
    data = list()
    if debug:
        for arg in args:
    ...
    if options.outfile is None:
        outfile = os.path.join(options.prefix, (
            bamfile.replace(".bam", ".splices.pickle") + "." + st))
    else:
...
if __name__ == "__main__":
    ...
    parser.add_option("--splice_type", dest="splicetypes", default=None,
                      action="append")
    parser.add_option("--slop", dest="slop", default=0,
                      help=SUPPRESS_HELP)#help="alignment slop tolerance (for overhangs)") #not implemented
    ...
    parser.add_option("--exitOnBuild", dest="exitOnBuild", default=False,
                      action="store_true")
