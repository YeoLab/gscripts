...
def get_regions():
    ...
    """
    path = "/nas3/yeo/nas3/yeolab/Genome/ensembl/AS_STRUCTURE/hg19data4/"
    #don't know if exons are coding exons or CDS
    UTR3 = pybedtools.BedTool(os.path.join(path, "UTR3_hg19_frea_sorted.withscore"))

    #also want just the first and last UTR3 element for each transcript...
    
    UTR5 = pybedtools.BedTool(os.path.join(path, "UTR5_hg19_frea_sorted.withscore")) 
    ...
    exons = pybedtools.BedTool(os.path.join(path, "exon_hg19_frea_sorted.withscore")) 

    
    #transcription_stop = pybedtools.BedTool(os.path.join(path, "polya_hg19_frea_sorted.withscore") 
    ...
    for interval in UTR3:
        if interval.strand == "+":
            if interval.name not in transcription_stop:
                transcription_stop[interval.name] = {"interval" : interval, "stop_codon" : interval.end}
            transcription_stop[interval.name]['stop_codon'] = max(transcription_stop[interval.name]['stop_codon'], 
                                                                  interval.end)
        else: #strand == -
            if interval.name not in transcription_stop:
                transcription_stop[interval.name] = {"interval" : interval, "stop_codon" : interval.start}
            transcription_stop[interval.name]['stop_codon'] = min(transcription_stop[interval.name]['stop_codon'], 
                                                                  interval.start)
            
    transcription_stop = pybedtools.BedTool([[interval['interval'].chrom, 
                                              interval['stop_codon'], 
                                              interval['stop_codon'],  
                                              interval['interval'].name, 
                                              interval['interval'].score, 
                                              interval['interval'].strand] 
                                             for interval in transcription_stop.values()]).saveas()
    ...
    region['stop_codons'] = stop_codons
