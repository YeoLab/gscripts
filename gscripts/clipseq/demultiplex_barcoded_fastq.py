...
def reformat_read(name, seq, plus, quality, barcodes):
    ...
    for barcode_length in sorted(barcode_lengths, reverse=True):
        ...
        cur_barcode = randomer[
                      RANDOMER_FRONT_LENGTH:adapter_length - RANDOMER_BACK_LENGTH]
...
if __name__ == "__main__":
    ...
    parser.add_option("-f", "--fastq", dest="fastq",
                      help="fastq file to barcode seperate")
    parser.add_option("-b", "--barcodes", dest="barcodes",
                      help="file of barcode / barcode id")
    ...
    with open(options.fastq) as fastq_file, open(options.metrics_file,
                                                 'w') as metrics_file:
        while True:
            try:
                ...
                barcode, randomer, result = reformat_read(name, seq, plus,
                                                          quality, barcodes)
